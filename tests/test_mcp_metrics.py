...
_use_sqlite = patch("onec_help.mcp_metrics._use_redis", return_value=False)


def test_use_redis_true_when_mcp_metrics_db_unset() -> None:
    """_use_redis returns True when MCP_METRICS_DB is not set."""
    from onec_help.mcp_metrics import _use_redis
...
def test_record_request_redis_fails_fallback_sqlite(tmp_path: Path) -> None:
    ...
    with (
        patch("onec_help.mcp_metrics._use_redis", return_value=True),
        patch("onec_help.redis_cache.mcp_request_record", side_effect=RuntimeError),
        patch("onec_help.mcp_metrics._metrics_db_path", return_value=db),
    ):
        record_request("test_tool", True)
    with (
        patch("onec_help.mcp_metrics._use_redis", return_value=False),
        patch("onec_help.mcp_metrics._metrics_db_path", return_value=db),
    ):
        m = get_metrics()
    assert m["total"] == 1
...
def test_get_metrics_redis_fails_fallback_sqlite(tmp_path: Path) -> None:
    ...
    with _use_sqlite, patch("onec_help.mcp_metrics._metrics_db_path", return_value=db):
        record_request("x", True)
    with (
        patch("onec_help.mcp_metrics._use_redis", return_value=True),
        patch(
            "onec_help.redis_cache.mcp_metrics_get",
...
def test_record_request_sqlite_handles_connect_error() -> None:
    ...
    with (
        _use_sqlite,
        patch("onec_help.mcp_metrics._metrics_db_path", return_value="/nonexistent/dir/mcp.db"),
        patch("onec_help.mcp_metrics.sqlite3.connect", side_effect=OSError("readonly")),
    ):
        record_request("x", True)
...
def test_get_metrics_when_path_empty_returns_zeros() -> None:
    ...
    with (
        patch("onec_help.mcp_metrics._use_redis", return_value=False),
        patch("onec_help.mcp_metrics._metrics_db_path", return_value=""),
    ):
        m = get_metrics()
    assert m["total"] == 0
    assert m["last_hour"] == 0
...
def test_get_metrics_handles_conn_error() -> None:
    ...
    with (
        patch("onec_help.mcp_metrics._use_redis", return_value=False),
        patch("onec_help.mcp_metrics._metrics_db_path", return_value="/tmp/mcp_metrics.db"),
        patch("onec_help.mcp_metrics.os.path.isfile", return_value=True),
        patch(
            "onec_help.mcp_metrics.sqlite3.connect", side_effect=sqlite3.OperationalError("locked")
        ),
    ):
        m = get_metrics()
    assert m["total"] == 0
