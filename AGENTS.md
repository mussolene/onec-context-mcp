...
- **Только справка и память:** ingest выполнен, Qdrant доступен → `get_1c_help_index_status` показывает топики и memory → сценарии: `get_1c_code_answer`, `search_1c_memory` (по запросу — только стандарты/сниппеты), `search_1c_help_keyword`, `get_1c_help_topic`, `get_1c_function_info`, `save_1c_snippet`, `compare_1c_help`, `get_module_info`, `get_form_metadata` (при подходящем XML).
- **Справка + метаданные конфигурации:** выгрузка в ONEC_CONFIG_SOURCE_DIR, выполнен `metadata-graph-build` (или watchdog) → в `get_1c_help_index_status` видна коллекция onec_config_metadata. Поиск **семантический** (запрос естественным языком, напр. «документ реализация товаров») + подстрока по имени/синониму. `config_version` опционален, если в графе одна версия (подставляется автоматически). При выгрузке «в файлы» краулер читает из XML объекта синоним и реквизиты/табличные части — они возвращаются в `get_1c_metadata_object`. Сценарии: `search_1c_metadata`, `get_1c_metadata_object`, `get_1c_context_bundle`.
