...
- **Structured help source:** primary route для `build-api-structured` — **unpacked HTML**. Канонический persistent слой — `data/help_structured/*.jsonl`; `data/unpacked` использовать только как временный или ручной промежуточный каталог. План миграции и целевые stop-conditions: [docs/reference/structured-help-jsonl-first-plan.md](docs/reference/structured-help-jsonl-first-plan.md).
