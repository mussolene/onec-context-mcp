...
- **Рекомендуемый порядок вызовов:**
  ...
  2. Точный API/идентификатор — `get_1c_api_answer(name)`; structured truth-source — `get_1c_api_object(name)`; broad structured lookup — `search_1c_api(query)`; natural-language factual question — `answer_1c_help_question(question)`.
