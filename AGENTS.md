...
- **Рекомендуемый порядок вызовов:**
  ...
  2. Точный API/идентификатор — `search_1c_help_keyword` с полным именем (`Тип.Метод`), затем `get_1c_help_topic(topic_path)` по `path` из результата.
  3. Неточный вопрос по коду — `get_1c_code_answer`.
  4. Нужны стандарты/сниппеты явно — `search_1c_memory(query, domains="standards,snippets")`.
  5. Нужны объекты конфигурации — `search_1c_metadata` → `get_1c_metadata_object`.
...
- **Почему в ответах нет сниппетов и стандартов:** блок «Из памяти» (примеры кода, стандарты v8std, community_help) формируется из коллекции Qdrant **onec_help_memory**. Её заполняют команды **load-snippets** (SNIPPETS_DIR, parse-fastcode/parse-helpf) и **load-standards** (v8-code-style, v8std). Если эти команды не запускались или завершились с ошибкой (например, нет embedding API), коллекция пуста и в `get_1c_code_answer` / `search_1c_help(include_user_memory=True)` сниппеты и стандарты не появятся. Проверка: `get_1c_help_index_status` — в выводе должна быть строка «Memory (**onec_help_memory**): **N** points». При N=0 или отсутствии такой строки выполните `make load-snippets` и `make load-standards` (или через init/watchdog); убедитесь, что EMBEDDING_API_URL доступен при загрузке.
- **Блок памяти в ответах:** в `get_1c_code_answer` блок «Из памяти» упорядочен: сначала до 2 блоков [стандарт], затем до 2 [пример], затем community_help и остальное (всего до 6). Чтобы гарантированно получить контекст по стандартам и сниппетам, дополнительно вызывайте **search_1c_memory(query, domains="standards,snippets")** — инструмент ищет только в памяти и возвращает блоки с метками [стандарт] / [пример].
...
- **Только справка и память:** ingest выполнен, Qdrant доступен → `get_1c_help_index_status` показывает топики и memory → основной AI-маршрут: `get_1c_quick_guide`, `get_1c_api_answer` (точный API), `get_1c_code_answer`, `search_1c_memory`, `search_1c_help_keyword`, `get_1c_help_topic`, `get_1c_function_info`, `compare_1c_help`, `get_module_info`, `get_form_metadata`. `save_1c_snippet` — только для реально переиспользуемого и проверенного кода.
- **Справка + метаданные конфигурации:** выгрузка в ONEC_CONFIG_SOURCE_DIR, выполнен `metadata-graph-build` (или watchdog) → в `get_1c_help_index_status` видна коллекция onec_config_metadata. Поиск **семантический** (запрос естественным языком, напр. «документ реализация товаров») + exact/startswith-first lookup по имени. `config_version` опционален, если в графе одна версия (подставляется автоматически). При выгрузке «в файлы» краулер читает из XML объекта синоним и реквизиты/табличные части — они возвращаются в `get_1c_metadata_object`. Сценарии: `search_1c_metadata`, `get_1c_metadata_object`, `get_1c_task_context`; `get_1c_context_bundle` — legacy broad-context.
...
`search_1c_help_keyword` (для точного API) или `get_1c_code_answer` (для неточного вопроса) → реализация → внешний `document_diagnostics` → при ERROR/WARNING: исправить и повторить diagnostics до чистоты → `save_1c_snippet` только если код действительно переиспользуемый → опционально unit-тест 1С (YaxUnit) или BDD/сценарий (Vanessa-Automation). См. `docs/1c-testing-guide.md`.
...
- **Python (onec_help):** `PYTHONPATH=src python3 -m pytest tests -v --cov=src/onec_help --cov-report=term-missing --cov-fail-under=77`; `ruff check src tests && ruff format --check src tests`. При падении покрытия — добавить тесты. Для проверки инструментов на живом MCP: поднять сервисы (`make up` и т.д.), затем `MCP_INTEGRATION=1 PYTHONPATH=src python3 -m pytest tests/test_mcp_integration.py tests/test_mcp_functional_crypto.py -v --no-cov` (см. `docs/mcp-1c-help-verification-report.md`).
