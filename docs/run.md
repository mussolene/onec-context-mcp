...
2. **Размерность векторов:** должна совпадать с коллекцией. Задайте **EMBEDDING_DIMENSION** в соответствии с моделью в LM Studio (например 384 для paraphrase-multilingual, 1024 для mxbai-embed-large). Если коллекция уже создана с другой размерностью — пересоздайте индекс (`reinit --force`) или укажите правильную размерность до первого ingest.
