...
В Docker контейнер MCP запускается через **быстрый entry point** `python -m onec_help.mcp_server` (без загрузки всего CLI), чтобы порт 8050 открывался быстрее. Если MCP «лежит» и подключение невозможно:
...
Локально тот же быстрый старт: `python -m onec_help.mcp_server /data --transport streamable-http --host 0.0.0.0 --port 8050`.
