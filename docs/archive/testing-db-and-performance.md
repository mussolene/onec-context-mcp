...
| **CLI** (load-snippets, load-standards, …) | В `tests/test_cli.py` мокается **`onec_help.interfaces.cli._get_memory_store`** (нет записи в Qdrant). В **conftest** для **test_cli** включён **redis_mock_for_ingest**: используется fakeredis, чтобы cmd_load_snippets/cmd_load_standards не писали в реальный Redis (snippets_cache, standards_run_record). |
