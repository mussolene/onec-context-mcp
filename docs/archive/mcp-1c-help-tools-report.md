...
### 2.1. MCP 1c-help (16 инструментов)
...
| **get_form_metadata** | Разбор Form.xml: атрибуты и команды. | `xml_content` (обяз.). | xml_content до 64 KB. | Передать полный XML формы (с xmlns). Ожидаются элементы Attribute и Command (локальные имена). | Усечённый XML или иная разметка (напр. только FormAttribute) → пустые атрибуты/команды. |
...
| **search_1c_metadata** | Поиск объектов конфигурации 1С по имени (Документы, Справочники, Регистры и т.д.). | `query` (обяз.), **`config_version`** (обяз.), `object_type=None`, `limit=20`. | — | После `metadata-graph-build` для выгрузки. Пример: `query="Документ"`, `config_version` из выгрузки/проекта. | Без графа — «No metadata objects found». config_version обязателен. |
| **get_1c_metadata_object** | Детали объекта метаданных по идентификатору. | `object_id` (обяз.). | — | object_id из выдачи search_1c_metadata (напр. Document.Sales). | При пустом графе или неизвестном id — «Metadata object not found». |
| **get_1c_context_bundle** | Комбинированный контекст: справка + память + объекты метаданных для задачи. | `query`, `config_version=None`, `file_uri=None`, `symbol_name=None`, `limit=5`. | — | Один вызов для подготовки ответа; при необходимости передать config_version для подбора объектов конфигурации. | При пустом графе метаданных возвращает только справку и память. |
...
## 9. Безопасность MCP и Qdrant: auth, TLS, reverse proxy

### MCP HTTP (порт 8050)

- MCP по HTTP **не имеет встроенной аутентификации** и рассчитан только на доверенную сеть (localhost/VPN).
- Рекомендуемый вариант для HTTP‑транспорта:
  - Контейнер `mcp` слушает **только localhost** на хосте (`127.0.0.1:8050`) или только внутреннюю Docker‑сеть.
  - Снаружи открыт только reverse proxy (nginx/Traefik/Caddy) с:
    - TLS (сертификат, шифрование трафика);
    - аутентификацией (basic, OAuth2/OIDC, mTLS и т.п.).
  - Cursor и другие клиенты подключаются к URL reverse proxy; прямой доступ к контейнеру `mcp` снаружи отключён.
- Для сценариев «только локально» (Cursor на той же машине) достаточно публикации на `127.0.0.1:8050` без внешнего proxy, но порт не должен быть доступен с других машин.

### Qdrant (порт 6333)

- Qdrant содержит весь индекс справки, памяти и метаданных конфигурации и является **NDA‑критичным** хранилищем.
- По умолчанию рекомендуется:
  - не публиковать порт 6333 на хост вовсе (только внутренняя сеть Docker);
  - подключать `mcp` и `ingest-worker` к Qdrant по имени сервиса (`QDRANT_HOST=qdrant`, `QDRANT_PORT=6333`).
- Если требуется внешний доступ к Qdrant:
  - использовать TLS и аутентификацию, поддерживаемые самим Qdrant (API key, HTTPS) **или**
  - помещать Qdrant за reverse proxy с auth/TLS;
  - ограничивать доступ по сети (VPN, firewall, security‑groups).
- В сценариях с управляемым Qdrant (cloud‑версия) host/port/URL и ключи доступа настраиваются по документации Qdrant; переменные окружения приложения (QDRANT_HOST/QDRANT_PORT) должны указывать на защищённую точку входа.

### Redis

- Redis хранит кэш ingest, состояние watchdog и метрики MCP; по содержимому он менее чувствителен, чем Qdrant, но всё равно может раскрывать пути файлов и сообщения об ошибках.
- Для безопасности:
  - Redis не должен публиковаться на 0.0.0.0; доступ только из внутренней сети Docker или по VPN.
  - При необходимости удалённого доступа — использовать шифрованные туннели (SSH, VPN) и/или встроенную аутентификацию Redis, а не открытый порт в интернет.
