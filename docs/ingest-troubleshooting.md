...
### 2.8. Indexed vectors не растёт для коллекции с BM25

После **add-bm25** у коллекции (например onec_help_memory) в дашборде **Indexed vectors** может оставаться равным **Points** (например 4062/4062), тогда как у onec_help — примерно **2× Points** (dense + sparse). Это не ошибка дашборда и не баг: **indexed_vectors_count** возвращает сам Qdrant — число векторов, уже попавших в HNSW/sparse-индекс. Оптимизатор Qdrant строит индексы **асинхронно**; пока сегмент со sparse-векторами не собран, в счётчике учитываются только dense.

**Что смотреть в дашборде:** колонка **Status** (green / yellow / grey / red). **yellow** — оптимизация идёт; **grey** — можно запустить оптимизацию вручную (Qdrant Web UI → коллекция → «Trigger optimizers»). После завершения индексации ожидайте **Indexed vectors ≈ 2× Points** для коллекций с BM25.
...
### 2.10. add-bm25: после таймаута/сбоя «удалилась половина базы»

**Причина (старое поведение):** add-bm25 делал **recreate_collection** (коллекция сразу очищалась), затем записывал точки батчами. При таймауте или падении процесса в середине upsert в коллекции оставались только уже записанные точки — остальные терялись. Повторный запуск работал уже с урезанной коллекцией и мог снова упасть по таймауту, оставляя ещё меньше данных.

**Исправление:** с текущей версией add-bm25 пишет **сначала во временную коллекцию** `{name}_bm25_tmp`. Исходная коллекция удаляется только после того, как временная полностью заполнена и начинается «подмена» (копирование из tmp в целевую). При сбое во время записи в tmp исходная коллекция не трогается. При сбое во время подмены при следующем запуске add-bm25 автоматически достраивает копирование из `*_bm25_tmp` в целевую коллекцию (recovery).

**Если данные уже потеряны:** восстановить из бэкапа Qdrant (`make qdrant-restore` из снапшота) и заново выполнить `make add-bm25` с увеличенным `QDRANT_TIMEOUT`.

### 2.11. add-bm25: Error 137 (SIGKILL, OOM)

**Симптомы:** `make add-bm25` обрывается с `make: *** [add-bm25] Error 137` после вывода вроде `[add-bm25] Upserted 100000/117696 to tmp`. В дашборде остаётся временная коллекция `onec_help_bm25_tmp` с частью точек.

**Причина:** код выхода **137** = 128 + 9 = **SIGKILL**. В Docker/Linux процесс чаще всего убивает **OOM killer** из‑за нехватки RAM. add-bm25 загружает в память **все точки** коллекции (scroll), строит BM25-векторы для всего корпуса и пишет во временную коллекцию — при ~118k точек и 768-мерных векторах пиковая память может составлять несколько гигабайт.

**Что проверить:**

1. **Логи контейнера mcp** (в них может быть запись об OOM):
   ```bash
   docker compose -f docker-compose.base.yml -f docker-compose.yml logs mcp --tail 100
   ```
2. **Память во время add-bm25:** `docker stats` в отдельном терминале — смотрите использование RAM контейнером `mcp` и общее по системе.

**Что сделать:**

- **Увеличить лимит памяти контейнеру mcp** в `docker-compose.base.yml` (или переопределить в `docker-compose.override.yml`):
  ```yaml
  mcp:
    mem_limit: 6g   # или 8g при 118k+ точек
  ```
  Перезапустить: `docker compose up -d mcp`, затем снова `make add-bm25`.

- **Запускать add-bm25 локально** (вне Docker), если на хосте больше RAM: `python -m onec_help add-bm25` при настроенных `QDRANT_HOST`/`QDRANT_PORT` и доступе к Qdrant.

- **Повторный запуск:** данные не теряются — исходная коллекция `onec_help` не тронута, временная `onec_help_bm25_tmp` уже содержит часть точек. При следующем `make add-bm25` код делает **recovery**: обнаруживает существующую `*_bm25_tmp`, достраивает копирование из неё в целевую коллекцию (см. §2.10). Поэтому после увеличения памяти достаточно снова выполнить `make add-bm25`.
