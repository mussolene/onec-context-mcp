...
2. **Один запрос с кодом** — `get_1c_code_answer("как прочитать JSON в Соответствие")` или `get_1c_code_answer("Запрос.ВыполнитьПакет", code_only=True)`. Должен вернуть топики и/или блоки кода без ошибки размерности (384/768).
