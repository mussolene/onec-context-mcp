...
4. Редактировать по одному файлу.
5. После каждой правки вызывать `document_diagnostics(uri)`.
6. **При ERROR:** исправить → повторить п. 5.
7. После batch правок: вызвать `did_change_watched_files` для переиндексации LSP.
8. Повторить п. 4–7 для следующего файла.
