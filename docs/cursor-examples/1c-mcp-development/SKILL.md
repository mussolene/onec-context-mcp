...
| Неточный вопрос по коду | 1c-help | `get_1c_code_answer` | Основной tool для примеров и платформенного контекста. |
...
| Явные стандарты/сниппеты | 1c-help | `search_1c_memory` | `domains="standards,snippets"`. |
| Объекты конфигурации | 1c-help | `search_1c_metadata`, `get_1c_metadata_object` | Подключать только когда нужен объектный контекст. |
...
2. Если запрос точный: `get_1c_api_answer("Тип.Метод")`; если неточный: `get_1c_code_answer(query)`.
...
5. Если нужны стандарты/сниппеты явно: `search_1c_memory(query, domains="standards,snippets")`.
