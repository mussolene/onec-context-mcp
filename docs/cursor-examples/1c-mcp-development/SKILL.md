...
| Точный API / идентификатор | 1c-help | `search_1c_help_keyword` | Передавать полное имя `Тип.Метод`. |
...
2. Если запрос точный: `search_1c_help_keyword("Тип.Метод")`; если неточный: `get_1c_code_answer(query)`.
3. Если нужен полный текст: `get_1c_help_topic(topic_path)` по `path` из поиска.
4. Если нужны стандарты/сниппеты явно: `search_1c_memory(query, domains="standards,snippets")`.
5. Реализовать или адаптировать код.
6. Вызвать внешний `document_diagnostics(uri)`.
7. **При ERROR или WARNING:** исправить → повторить п. 6.
8. `save_1c_snippet` использовать только если код действительно переиспользуемый и уже проверен diagnostics.
