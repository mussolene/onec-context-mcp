...
В этой папке — эталонные копии схем для инструментов, которые чаще всего расходились с кэшем клиента (имена параметров `uri_or_path`, `xml_content`, поля `get_1c_context_bundle`). Их можно сравнить с локальным кэшем или использовать как ориентир при ручной правке.
