...
- **Использование в версии** — с какой версии платформы доступна функция/метод. Контент в элементах `p.V8SH_versionInfo`, следующих за заголовком `p.V8SH_chapter` с текстом «Использование в версии:». Может быть несколько абзацев (например: «Доступен с 8.2», «Описание изменено в 8.3.24»). В structured JSONL это поле **`platform_since`**, отдельно от **`availability`** (только «Доступность») и от **`version`** (версия снимка справки в пути индекса). Строка вида `[method, 8.3.13]` сразу под `h1.V8SH_pagetitle` сохраняется как **`page_descriptor`** (в MD — строка под заголовком `#`).
