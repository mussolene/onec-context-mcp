...
Для быстрого старта используйте OpenAI-compatible backend с локальным Ollama или LM Studio. Меняйте модель только вместе с переиндексацией, чтобы размерность и семантика индекса совпадали с поиском.
