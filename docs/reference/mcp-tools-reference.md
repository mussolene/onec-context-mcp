...
**Поиск:** `search_1c_metadata_exact` делает exact-first lookup по `id`, `name`, `full_name`, `path` внутри `config_version`. **Канонический id в индексе** — **`EnglishType/ИмяОбъекта`** (например `Document/РеализацияТоваровУслуг`): это формат выгрузки KD2/Qdrant, **не** синтаксис языка запросов 1С (в запросах виртуальные таблицы задаются как **`Документ.Имя`**, **`Справочник.Имя`** и т.д., без `Document/`). В коде на BSL к объектам метаданных обращаются через коллекции **`Метаданные.Документы.Имя`**, **`Метаданные.Справочники.Имя`**, **`Метаданные.ПланыВидовХарактеристик.Имя`** и т.п.; MCP нормализует и **единственное число** (`Документ.Имя`), и **множественное** (`Документы.Имя`), и цепочку с корнем **`Метаданные.`**; при копировании пути с хвостом **`.Реквизиты`**, **`.СтандартныеРеквизиты`**, **`.ТабличныеЧасти`** и т.п. для поиска объекта используется только **имя объекта** до следующей точки. Английские префиксы (`Document.Имя`) тоже приводятся к id со слэшем. `search_1c_metadata_semantic` сочетает векторный поиск с prepended exact для коротких имён и путей `Type/Name`; для узкого поиска задавайте **`object_type`** (`Document`, `Catalog`, `InformationRegister`, `ChartOfCharacteristicTypes`, …). `search_1c_metadata_fields` ищет по **имени или синониму поля** в реквизитах, измерениях/ресурсах регистров, стандартных свойствах, колонках табличных частей и командах.
