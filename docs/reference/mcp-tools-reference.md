...
**Поиск:** `search_1c_metadata_exact` делает exact-first lookup по `id`, `name`, `full_name`, `path` внутри `config_version`. **Канонический id в индексе** — **`EnglishType/ИмяОбъекта`** (например `Document/РеализацияТоваровУслуг`): это соглашение KD2/Qdrant, **не** синтаксис языка запросов (в запросах — **`Документ.Имя`**, **`Справочник.Имя`** и т.д.).

В BSL дерево метаданных обычно задаётся как **`Метаданные.Документы.Имя`**, **`Метаданные.Справочники.Имя`**, **`Перечисления.ИмяПеречисления.ИмяЗначения`** (глобально читаемые перечисления) и т.п.; к менеджеру объекта обращаются цепочками вроде **`Документы.Имя.СоздатьДокумент()`**, **`НайтиПоНомеру`**, **`Реквизиты`**, **`ТабличныеЧасти`**. В типовой конфигурации **имя объекта метаданных — идентификатор без точки**; для поиска в графе конфигурации MCP берёт **первый сегмент после префикса типа** (одинаково для `Документы.Авансы` и `Справочники.Авансы` — различаются префиксом типа). Повтор **`Метаданные.`**, корни **`Metadata.`**, пробелы вокруг точек и варианты **`Глобальный контекст.…`** при нормализации учитываются; для EN-клиента — коллекции вроде **`Documents.Имя`**. Если в конфигурации **одно и то же имя** встречается у документа и справочника, задайте **`object_type`**. Семантика платформенной модели «объект метаданных» (свойства, методы) — через справку: **`get_1c_api_answer("Глобальный контекст.Метаданные")`**, **`search_1c_api("ОбъектМетаданных")`**.
